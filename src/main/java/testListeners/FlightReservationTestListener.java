...
public class FlightReservationTestListener implements ITestListener {
	...
	public void onTestSuccess(ITestResult result) {
		...
		try {
			TestReportFormatter.writeReport(result.getName(), result.getParameters().toString(), "PASSED", result.getThrowable().getMessage(), TestCase.FLIGHT_RESERVATION);
		} catch (Exception e) {
	...
	public void onTestFailure(ITestResult result) {
		...
		try {
			...
			TestReportFormatter.writeReport(result.getName(), result.getParameters().toString(), "FAILED", result.getThrowable().getMessage(), TestCase.FLIGHT_RESERVATION);
	...
	public void onStart(ITestContext context) {
		...
		try {
			TestReportFormatter.initialize(TestCase.FLIGHT_RESERVATION);
	...
	public void onFinish(ITestContext context) {
		...
		for(String[] reports : report) {
			try {
				PostGreSQLHandler.pushReportToDB(reports, DataBaseTable.RESERVATION_REPORT);
