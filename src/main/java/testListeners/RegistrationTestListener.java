...
public class RegistrationTestListener implements ITestListener {
	...
	public void onTestSuccess(ITestResult result) {
		...
		try {
			TestReportFormatter.writeReport(result.getName(), result.getParameters().toString(), "PASSED", result.getThrowable().getMessage(), TestCase.REGISTRATION);
		} catch (Exception e) {
	...
	public void onTestFailure(ITestResult result) {
		...
		try {
			...
			TestReportFormatter.writeReport(result.getName(), result.getParameters().toString(), "FAILED", result.getThrowable().getMessage(), TestCase.REGISTRATION);
	...
	public void onStart(ITestContext context) {
		...
		try {
			TestReportFormatter.initialize(TestCase.REGISTRATION);
	...
	public void onFinish(ITestContext context) {
		...
		for(String[] reports : report) {
			try {
				PostGreSQLHandler.pushReportToDB(reports, DataBaseTable.REGISTRATION_REPORT);
